 ...
 * @subpackage request
 */
abstract class FormSaveController {
    /** @var modX $modx */
    public $modx;
     ...
     * @param array $config
     */
    function __construct(FormSave &$formsave,array $config = array()) {
        $this->formsave =& $formsave;
        $this->modx =& $formsave->modx;
    ...
    public function runProcessor($processor,array $scriptProperties = array()) {
        ...
        try {
            ...
            $this->modx->log(modX::LOG_LEVEL_ERROR,'[FormSave] '.$e->getMessage());
        }
        return $output;
    }
}